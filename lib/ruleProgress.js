...
var RuleProgress = (function() {
    return function() {
                ...
                default:
                    _.each(scopeList, function(scope) {
                        ...
                        if (scope==='lar' || type==='special') {
                            this.progress.estimate += this._HMDA_JSON.hmdaFile.loanApplicationRegisters.length * 
                                rules.length;
                        } else if (type==='macro') {
                            // apply 99% confidence interval based off normal distribution curve calculated from 5 sample banks
                            var upperbound = 1.69;
                            this.progress.estimate += Math.floor(this._HMDA_JSON.hmdaFile.loanApplicationRegisters.length * 
         ...
         */
        this.postTaskCompletedMessage = function(moo) {
            if (moo === undefined) {
                this.progress.count ++;
            } else {
                this.progress.count += moo;
            }
