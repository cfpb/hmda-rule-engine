...
var handleUniqueULIErrors = function(counts) {
    var errors = [];
    var ulis = _.keys(counts);
    for (var i = 0; i < ulis.length; i++) {
        var uli = ulis[i];
        if (counts[uli].length > 1) {
            var error = {'properties': { 'universalLoanID': uli }};
            var lineNumbers = [];
            for (var j = 0; j < counts[uli].length; j++) {
                lineNumbers.push(counts[uli][j].lineNumber);
            }
            error.lineNumber = lineNumbers.join(', ');
            errors.push(error);
        }
...
module.exports = {
    ...
    handleUniqueULIErrors: handleUniqueULIErrors,
