...
var EngineCustomDataLookupConditions = (function() {
    return function() {
        ...
        this.isValidLEI = function(hmdaFile) {
            var ts = hmdaFile.transmittalSheet,
                agencyCode = ts.agencyCode,
                legalEntityID = ts.legalEntityID;
            return this.apiGET('isValidControlNumber',
                [agencyCode, legalEntityID])
            .then(function(response) {
                if (!utils.jsonParseResponse(response).result) {
                    return utils.handleArrayErrors(hmdaFile, [1], ['agencyCode', 'legalEntityID']);
                } else {
                    var lineNumbers = [],
                        lars = hmdaFile.loanApplicationRegisters,
                        len = lars.length;
                    ...
                    for (var i = 0; i < len; i++) {
                        var element = lars[i];
                        if (element.agencyCode !== agencyCode || element.legalEntityID !== legalEntityID) {
                            lineNumbers.push(i + 2);
                        }
                    }

                    if (lineNumbers.length !== 0) {
                        return utils.handleArrayErrors(hmdaFile, lineNumbers, ['agencyCode', 'legalEntityID']);
                    }
                }
                return true;
            });
